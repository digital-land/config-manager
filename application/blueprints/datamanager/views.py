...
def check_results(request_id):
    try:
        ...
        detailed_data = fetch_all_details(async_api, request_id)
        ...
        for row in detailed_data:
            ...
            if geom and isinstance(geom, dict) and geom.get("type"):
                ...
                wkt_str = (row.get("converted_row") or {}).get("WKT")
                geometry = None
                if wkt_str:
                    try:
                        ...
                        print(f"⚠️ Error parsing WKT for entry {row.get('entry_number')}: {e}")
        ...
        print("🚧 Geometries built:", json.dumps(geometries, indent=2))
