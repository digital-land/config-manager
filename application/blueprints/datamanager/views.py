    ...
    if request.method == 'POST':
        ...
        if mode == 'final':
            ...
            errors.update({
                ...
                'organisation': (
                    org_warning or
                    (selected_orgs and organisation not in selected_orgs)
                ),
            ...
            if not any(errors.values()):
                ...
                # ✅ Send to backend
                try:
                    async_api = "http://localhost:8000/requests"
                    ...
                    if response.status_code == 202:
                        ...
                        return redirect(url_for("datamanager.check_results", request_id=request_id, organisation=organisation))
                    else:
...
def check_results(request_id):
    try:
        ...
        details_response = requests.get(f"http://localhost:8000/requests/{request_id}/response-details")
        ...
        if details_response.status_code == 200:
            try:
                ...
                print("✅ detailed_data loaded:", json.dumps(detailed_data[:1], indent=2))
            except Exception as e:
