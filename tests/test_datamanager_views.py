...
import pytest
from application.blueprints.datamanager.views import get_spec_fields_union, read_raw_csv_preview
...
class TestDatamanagerViews:
    ...
    @patch('requests.post')
    @patch('requests.get')
    def test_dashboard_add_post_success_stores_session(self, mock_get, mock_post, client):
        ...
        response = client.post('/datamanager/dashboard/add', data={
            'mode': 'final',
    ...
    def test_add_data_post_updates_session(self, mock_get, mock_post, client):
        ...
        response = client.post('/datamanager/check-results/test-id/add-data', data={
    ...
    def test_optional_fields_submit(self, mock_post, client):
        ...
        response = client.post('/datamanager/check-results/optional-submit', data={
            'request_id': 'test-id',
            'documentation_url': 'https://test.gov.uk'
        })
        assert response.status_code == 302
    ...
    def test_dashboard_add_get_orgs_for(self, mock_get, client):
        ...
        mock_get.side_effect = [
            MagicMock(json=lambda: {"datasets": [{"name": "test-dataset", "dataset": "test-id", "collection": "test-collection"}]}),
            MagicMock(json=lambda: {"rows": [{"organisation": {"label": "Test Org", "value": "local-authority:TEST"}}]})
        ]
    ...
    def test_configure_post(self, mock_get, mock_post, client):
        ...
        response = client.post('/datamanager/configure/test-id', data={
            'map_raw[col1]': 'reference',
            'geom_type': 'point'
        })
        assert response.status_code == 302
